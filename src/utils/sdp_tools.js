...
const makeLine = function (type, obj, location) {
  const str = obj.format instanceof Function ? (obj.format(obj.push ? location : location[obj.name])) : obj.format
  const args = [type + '=' + str]
...
const parseReg = function (obj, location, content) {
  ...
  const keyLocation = obj.push ? {} : needsBlank ? location[obj.name] : location // otherwise, named location or root
...
const SDPTools = {
  ...
  adjustMLineOrder: function (session, order) {
    if (order) {
      ...
      for (let i = 0; i < session.media.length; i++) {
        const type = session.media[i].content || session.media[i].type
  ...
  removeCodecByPayload: function (session, index, payloads) {
    ...
    const mediaSession = session.media[index]
    ...
    if (mediaSession.payloads && mediaSession.payloads.split) {
      const sessionPayloads = mediaSession.payloads.split(' ')
    ...
    codecPtList.map(item => {
      mediaSession.fmtp.forEach(itemIn => {
        if (itemIn.config.match('apt=' + item)) {
          const rtxPtIndex = payloads.indexOf(itemIn.payload)
          if (rtxPtIndex >= 0) {
  ...
  removeInvalidCode: function (sdp) {
    const parsedSdp = SDPTools.parseSDP(sdp)
    ...
    for (let i = 0; i < parsedSdp.media.length; i++) {
      ...
      if (media.type === 'video') {
        if (media.payloads && media.payloads.split) { // There is no split if only one PT
          const mediaPayloads = media.payloads.split(' ')
          ...
          if (media.fmtp && media.fmtp.length) {
            for (let j = 0; j < media.fmtp.length; j++) {
              const fmtpPT = String(media.fmtp[j].payload)
              let aptPT
          ...
          if (media.rtp && media.rtp.length) {
            for (let K = 0; K < media.rtp.length; K++) {
              const rtpPT = String(media.rtp[K].payload)
          ...
          if (media.rtcpFb && media.rtcpFb.length) {
            for (let K = 0; K < media.rtcpFb.length; K++) {
              const rtcpFbPT = String(media.rtcpFb[K].payload)
  ...
  getRTCRtpCapabilities: function (sdp, excludeList) {
    ...
    const capabilities = { haveAudio: false, haveVideo: false }
    // Only one video media line is parsed
    let hasVideoGet = false
    const session = SDPTools.parseSDP(sdp)
    if (session && session.media && session.media.length) {
      for (let i = 0; i < session.media.length; i++) {
        ...
        if (media.type === 'audio') {
          ...
          const payloads = media.payloads && media.payloads.split(' ')
  ...
  setH264Resolution: function (session, index, height) {
    ...
    const levelIdc = this.resolutionToLevelIdcMap[height] || '16'
    const h264Codec = this.getCodecByName(session, index, ['H264'])
    const media = session.media[index]

    if (media) {
      const fmtp = media.fmtp
      if (fmtp.length > 0) {
        for (let i = 0; i < fmtp.length; i++) {
          const index = fmtp[i].config.indexOf('profile-level-id')
          if (index >= 0) {
            const str = fmtp[i].config.substr(index, 21)
            const replacement = str + levelIdc
  ...
  setVp8Resolution: function (session, index, width, height) {
    ...
    const mbWidth = (parseInt(width, 10) + 15) / 16
    const mbHeight = (parseInt(height, 10) + 15) / 16
    const maxFs = Math.floor(mbWidth) * Math.floor(mbHeight)

    const vp8Codec = this.getCodecByName(session, index, ['VP8'])
    const media = session.media[index]
    if (media) {
      const fmtp = media.fmtp
      for (let i = 0; i < fmtp.length; i++) {
        const index = fmtp[i].config.indexOf('max-fs')
        if (index >= 0) {
          const replacement = 'max-fs=' + maxFs
  ...
  getH264Resolution: function (session, index) {
    ...
    if (media) {
      for (let i = 0; i < media.fmtp.length; i++) {
        const index = media.fmtp[i].config.indexOf('profile-level-id=')
        if (index >= 0) {
          const levelId = media.fmtp[i].config.match('profile-level-id=[a-zA-Z0-9]{6}')[0].split('=')[1]
  ...
  getVp8Resolution: function (session, index) {
    ...
    if (media) {
      for (let i = 0; i < media.fmtp.length; i++) {
        const index = media.fmtp[i].config.indexOf('max-fs=')
        if (index >= 0) {
  ...
  setFrameRate: function (session, index, framerate) {
    ...
    if (media) {
      if (media.framerate) {
        ...
        const fmtp = media.fmtp
        for (let i = 0; i < fmtp.length; i++) {
          const index = fmtp[i].config.indexOf('max-fr')
          if (index >= 0) {
            const replacement = 'max-fr=' + framerate
  ...
  getFramerate: function (session, index) {
    ...
    if (media) {
      if (media.framerate) {
        ...
        const fmtp = media.fmtp
        for (let i = 0; i < fmtp.length; i++) {
          const pos_max_fr = fmtp[i].config.indexOf('max-fr')
          if (pos_max_fr >= 0) {
  ...
  setMediaBandwidth: function (session, index, bitRate) {
    ...
    const bandwidth = { AS: bitRate / 1000 + 192, TIAS: bitRate }
    const media = session.media[index]
    if (media) {
  ...
  setXgoogleBitrate: function (session, bitrate, index) {
    if (index || index === 0) {
      ...
      for (let i = 0; i < session.media.length; i++) {
        const h264Codec = this.getCodecByName(session, i, ['H264'])
        const media = session.media[i]
        this.setXgoogle(media, h264Codec, bitrate)
      }
    }
  ...
  removeRembAndTransportCC: function (session, index) {
    const deleteRembTransportCC = function (media) {
      if (media.rtcpFb && media.rtcpFb.length) {
        for (let j = 0; j < media.rtcpFb.length; j++) {
          const type = media.rtcpFb[j].type
  ...
  addCandidate: function (media, candidate) {
    ...
    const content = candidate.slice(2)
    for (let j = 0; j < (grammar[type] || []).length; j += 1) {
      const obj = grammar[type][j]
      if (obj.reg.test(content)) {
        const keyLocation = {}
  ...
  splitSDP: function (SDP) {
    const sdpArray = []
    const session = this.parseSDP(SDP)
